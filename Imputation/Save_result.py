...
def save_results(mads_mean,mads_std, mses_mean, mses_std, center_name):
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        x_labels = ['Zero Imputation', 'Mean Imputation', f'Iterative Imputation\n(1 Iteration)', f'Multiple Imputation\n(5 Iterations)', f'Multiple Imputation\n(20 Iterations)',
        ...
        plt.title(f"Imputation Techniques Comparison in {center_name} Data (MAD)", fontsize=26)  # 12
        ...
        plt.savefig(f'Imputation Techniques Comparison in {center_name} Data (MAD).tiff', dpi=300, format='tiff')
        plt.show()
        plt.close()
        ...
        plt.subplots_adjust(left=0.3)
        # pdf.savefig(fig, bbox_inches='tight')
        plt.title(f"Imputation Techniques Comparison in {center_name} Data (RMSE)", fontsize=26)  # 12
        plt.xlim(left=np.min(mses_mean) * 0., right=np.max(mses_mean) * 1.7)
        # plt.yticks(xval)
        plt.yticks(xval, x_labels, fontsize=16)
        plt.xlabel("RMSE", fontsize=18)
        plt.savefig(f'Imputation Techniques Comparison in {center_name} Data (RMSE).tiff', dpi=300, format='tiff')
