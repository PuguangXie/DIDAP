...
def missing(centers):
    ...
    df.to_csv(f"Missingness Abstract with Hip Fracture Data.csv")
    return results
...
def visualize_missingness(centers, output_dir="missingness_visualization"):
    ...
    for center_name, center_data in centers.items():
        ...
        plt.title(f"Missing Data Distribution in {center_name} with Hip Fracture Data", fontsize=16)
        ...
        output_path = os.path.join(output_dir, f"Missing Data Distribution in {center_name} with Hip Fracture Data.tiff")
...
def missing_heatmap(center_name, center_data):
    ...
    plt.title(f"Missingness Heatmap in {center_name} with Hip Fracture Data", fontsize=20, pad=2)  # 12
    plt.savefig(f'Missingness Heatmap in {center_name} with Hip Fracture Data.tiff', dpi=300, format='tiff')
    # plt.show()
    plt.close()
